...
with lib; let
  cfg = config.augs.metal.template.yggdrasil;
in {
  options.augs.metal.template.yggdrasil.enable = mkEnableOption "enable yggdrasil's system configuration";
      ...
      users = {
        ${config.var.user} = {
          ...
          extraGroups =
            ...
            ++ (
              if config.augs.virtualisation.podman.enable
              then ["podman"]
              else []
            )
            ++ (
              if config.augs.virtualisation.virt-manager.enable
              then ["libvirtd"]
              else []
            );
          openssh = {
